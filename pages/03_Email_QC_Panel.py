...
# SME QC Panel — single page, compact iPad layout with robust column/mapping checks
...
# ---------- Helpers ----------
REQUIRED_KEYS = ["ID","Q_EN","OPT_EN","ANS_EN","EXP_EN","Q_TA","OPT_TA","ANS_TA","EXP_TA","QC_TA"]
...
def _split_opts(s: str):
    ...
    while len(raw) < 4: raw.append("")
    return raw[:4]

def mapping_complete(m: dict) -> bool:
...
def show_mapper(expanded=True):
    with st.expander("📥 Load bilingual file (.csv/.xlsx) & map columns", expanded=expanded):
...
if row_key not in ss:
    ...
    ss.edit_ans = ta_ans0 if ta_ans0 else A
    ss.edit_ex  = ta_ex0
...
b1, b2, _ = st.columns([1.2,1.4,2.5])
with b1:
