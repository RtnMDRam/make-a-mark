...
# SME-only compact editor: two buttons bottom, tight options, answer own row
...
/* Option grid tighter */
.optrow {margin-top:4px; margin-bottom:0; gap:8px;}
...
rk = f"{ss.qc_idx}"
# initialize per-row session values once
for k,v in [
    (f"q_{rk}", _txt(ta_q)),
    (f"a_{rk}", _txt(A)),
    (f"b_{rk}", _txt(B)),
...
with bL:
    if st.button("✅ வேலை முடிந்தது — QC கோப்பை சேமிக்க", use_container_width=True):
        _save_current()
        buf = io.BytesIO()
        ss.qc_work.to_excel(buf, index=False)
        buf.seek(0)
        st.download_button(
            "⬇️ QC Excel (.xlsx)", data=buf, file_name="qc_verified.xlsx",
